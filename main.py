...
def format_logs(logs: List[Task], logs_date: str) -> str:
    ...
    return "\n".join(
        [
            f"{datetime.fromisoformat(log.created_date).strftime('%I:%M %p')} {log.title}"
...
def main():
    delta_days = 0
    ticktick_client = TicktickClient(os.getenv("TT_USER"), os.getenv("TT_PASS"))

    quebec_timezone = ZoneInfo("America/Toronto")
    logs_date = (datetime.now(quebec_timezone) + timedelta(days=delta_days)).strftime("%Y-%m-%d")
